---
name: intermedia-information
description: Use when the user wants to inspect information@metooshoes.com or create safe reply drafts without sending email.
---

# Intermedia Information Mail
...
2. Use `list_information_messages` to identify recent messages that do not already have a generated draft.
3. Use `get_information_message` before drafting so the response is grounded in the full message.
4. Draft in a warm, concise professional tone. Do not invent order status, refunds, delivery dates, inventory, policies, or commitments.
5. Call `create_information_reply_draft` only when the user asked for a draft or an approved automation is running.
